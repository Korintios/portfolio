...
export default function Testimonials() {
    ...
    useEffect(() => {
        ...
        if (carousel && TESTIMONIALS.length >= 3) {
            carousel.style.setProperty('--testimonials', (TESTIMONIALS.length + 1).toString());
        }
    })
    ...
    return (
        <div className="testimonials-container w-full h-screen flex items-center justify-center">
            ...
            <div className="testimonials flex flex-col gap-7">
                ...
                <div className={"testimonials-reviews flex flex-row items-start justify-center gap-6" + (TESTIMONIALS.length >= 3 ? " testimonials-animation" : "")}>
                    {TESTIMONIALS.map((testimonial, index) => (
                        <div key={index} className={"flex flex-col gap-3 w-[785px] bg-white bg-opacity-15 p-3 rounded-md text-white " + (index === 0 ? "one" : "")}>
