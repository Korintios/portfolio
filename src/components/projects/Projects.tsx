...
export default function Projects() {
  ...
  return (
    <section className=' justify-center min-h-screen sm:h-screen text-white'>
				<h1 className="xl:px-20 py-10 uppercase text-center xl:text-left font-hubballi text-2xl md:text-5xl w-full">{t('title')}</h1>
        <Swiper
          slidesPerView={5}
          spaceBetween={30}
          centeredSlides
              ...
              <div className="relative z-20 p-5 rounded-lg text-white flex flex-col justify-between h-full">
                ...
                <div className="flex flex-col gap-2">
                  <div className='flex flex-col gap-2 text-start'>
                    <h1 className="font-hubballi text-2xl font-medium">{project.title}</h1>
                    <p className="font-poppins text-sm text-white/75">{project.description}</p>
                  </div>
                  <div className="flex flex-col gap-2 text-start">
                    <h2 className="font-hubballi text-xl font-normal text-white/50">Tecnologias Utilizadas</h2>
                    <div className="flex flex-wrap gap-2 pointer-events-none">
                      {project.tags.map((tag, index: number) => (
                        <div key={index} className="border-2 border-info bg-iconcard-gradient-hover text-[12px] px-2 py-1 rounded-full">{tag}</div>
                      ))}
                    </div>
                  </div>
                </div>
